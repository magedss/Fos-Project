...
void _main(void)
{
	...
	cprintf("	1: Try to allocate set of blocks with different sizes [all should fit]\n\n") ;
	{
		...
		for (int i = 0; i < numOfAllocs; ++i)
		{
			...
			for (int j = 0; j < allocCntPerSize; ++j)
			{
				...
				{
					//panic("alloc_block_xx #2.%d: WRONG! content of the block is not correct. Expected %d\n");
	...
	/*Check memory allocation*/
	{
		...
		if ((freeFrames - sys_calculate_free_frames()) < (expectedAllocNumOfPages + expectedAllocNumOfTables))
		{
			...
			is_correct = 0;
			panic("alloc_block_xx #3.%d: WRONG! content of the block is not correct. Expected %d\n");
	...
	/*Check WS elements*/
	{
		...
		if (chk != 1)
		{
			...
			panic("alloc_block_xx #4.%d: WRONG! content of the block is not correct. Expected %d\n");
