...
{
	e->nicevalue=nice_value;
	if(e->env_status!=ENV_NEW)
	{
		fixed_point_t temp=__mk_fix((long long) (e->recentcpu)/(long long)4);
																long long npr= PRI_MAX-(long long)fix_round(temp)-(long long)2*e->nicevalue;
																if(npr>PRI_MAX)
																	npr=PRI_MAX;
																if(npr<PRI_MIN)
																	npr=PRI_MIN;
...
int env_get_recent_cpu(struct Env* e)
{
	...
	//Your code is here
	//Comment the following line
	fixed_point_t temp=__mk_fix(curenv->recentcpu*100);
		return fix_round(temp);
	return 0;
}
int get_load_average()
{
	...
	fixed_point_t temp=__mk_fix(loadavg*100);
	return fix_round(temp);
