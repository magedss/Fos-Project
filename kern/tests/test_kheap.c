...
int test_kmalloc()
{
	...
	correct = 1 ;
	{
		// why this test pass the first condition ?
	...
	//test kheap_virtual_address after kmalloc and kfree [20%]
	{
		...
		for (i = startIndex ; i < startIndex + 4*Mega/PAGE_SIZE; ++i)
		{
			...
			if (retrievedVA != 0)
			{
				if (correct)
				{ correct = 0;
				cprintf("retrievedVA:%u\n",retrievedVA);

				cprintf("Wrong kheap_1virtual_address\n"); }
			}
		...
		for (i = startIndex + 4*Mega/PAGE_SIZE; i < startIndex + (7*Mega + 8*kilo)/PAGE_SIZE; ++i)
		{
			...
			{
				if (correct)
				{ correct = 0; cprintf("Wrong kheap_2virtual_address\n"); }
			}
		}
		//frames of 6 MB
		for (i = startIndex + (7*Mega + 8*kilo)/PAGE_SIZE; i < startIndex + (13*Mega + 8*kilo)/PAGE_SIZE; ++i)
		{
			...
			{
				if (correct)
				{ correct = 0; cprintf("Wrong kheap_3virtual_address\n"); }
			}
		}
		...
		for (i = startIndex + (13*Mega + 8*kilo)/PAGE_SIZE; i < startIndex + (13*Mega + 24*kilo)/PAGE_SIZE; ++i)
		{
			...
			{
				if (correct)
				{ correct = 0; cprintf("Wrong kheap_4virtual_address\n"); }
			}
		}
	}
	if (correct)	eval+=20 ;
