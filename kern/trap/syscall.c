...
uint32 sys_hardlimit()
{
	return curenv->UHARDLIMIT;
}
void sys_env_set_nice(uint32 nv){
	env_set_nice(curenv,nv);
}
...
void* sys_sbrk(int increment)
{
	...
	if(increment==0){
	    ...
	    if (increment > 0) {
	   	// cprintf("man8\n");
	   	env->UBRK=ROUNDDOWN(env->UBRK,PAGE_SIZE);
	    	// less than or equal?
	    	if( ROUNDUP(increment+env->UBRK,PAGE_SIZE)<env->UHARDLIMIT){
	        int num_pages = ROUNDUP(increment,PAGE_SIZE)/PAGE_SIZE;
//	        cprintf("s");
	    	uint32 rounded=ROUNDUP(increment,PAGE_SIZE);
	    	uint32 va=env->UBRK;
	    	uint32 *ptr=NULL;


	    	for(int i=0;i<num_pages;i+=1){
	    		get_page_table(env->env_page_directory,va,&ptr);
	    		if(ptr==NULL){
	    			//create
	    	...
	    	return (void*)oldbrk;
	      	...
	      	if( -increment+oldbrk>USER_HEAP_START){
	      		int kon=oldbrk;
	      		if((kon%PAGE_SIZE)){
	      			...
	      			return (void*)env->UBRK;}
	      				...
	      				increment+=now;
	      				//free this ubrk
	      			sys_free_user_mem(env->UBRK,PAGE_SIZE);
	      			}
	      		}
	      		while (increment!=0){
