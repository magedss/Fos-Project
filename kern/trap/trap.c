...
//			bool ok=1;
			if (fault_va <= KERNEL_HEAP_MAX&&fault_va >= KERNEL_HEAP_START){
						//cprintf("[%08s] user1 PAGE fault va %08x\n", curenv->prog_name, fault_va);

				sched_kill_env(curenv->env_id);}
			 if(!(pt_get_page_permissions(faulted_env->env_page_directory, fault_va)&PERM_AVAILABLE)&&fault_va <= USER_HEAP_MAX&&fault_va >= USER_HEAP_START){
			//				cprintf("[%s] user TABLE fault va %08x\n", curenv->prog_name, fault_va);
								//cprintf("[%08s] user333 PAGE fault va %08x\n", curenv->prog_name, fault_va);

							sched_kill_env(curenv->env_id);
						}

			 if((pt_get_page_permissions(faulted_env->env_page_directory,fault_va)&PERM_WRITEABLE)&(pt_get_page_permissions(faulted_env->env_page_directory,fault_va)&PERM_PRESENT)){
				env_page_ws_print(curenv);

					//cprintf("[%08s] user2 PAGE fault vaaaa %u \n", (pt_get_page_permissions(faulted_env->env_page_directory,fault_va)&PERM_PRESENT));

				sched_kill_env(curenv->env_id);

			}
			 if((pt_get_page_permissions(faulted_env->env_page_directory,fault_va)&PERM_PRESENT))
					sched_kill_env(curenv->env_id);
