...
int process_command(int number_of_arguments, char** arguments) {
	...
	for(int i=0;i<NUM_OF_COMMANDS;i++){
		if(strcmp(commands[i].name,arguments[0])==0&&(number_of_arguments-1==commands[i].num_of_args||(commands[i].num_of_args==-1&&number_of_arguments>=1))){
			return i;
		}
		else 	if(strcmp(commands[i].name,arguments[0])==0){
	...
	bool ok=0;
	for(int i=0;i<NUM_OF_COMMANDS;i++){
		for(int x=0,y=0;x<strlen(commands[i].name);x++){
			if(strlen(arguments[0])>=strlen(commands[i].name)){
				break;
			}
			if(commands[i].name[x]==arguments[0][y]){
				y++;
			}
			if(y==strlen(arguments[0])){
				ok=1;
				LIST_INSERT_HEAD(&foundCommands,&commands[i]);
				break;
			}
	...
	if(ok){
		...
		return CMD_INVALID;
	//panic("process_command is nojhjt implemented yet");
	return 0;
}
