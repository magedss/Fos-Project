...
void allocate_user_mem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 rounded=ROUNDUP(size,PAGE_SIZE);
	uint32 va=ROUNDDOWN(virtual_address,PAGE_SIZE);
	uint32 *ptr=NULL;
	for(int i=va;i<va+rounded;i+=PAGE_SIZE){
...
//	return;
	...
	//cprintf("free haha sui=========================WS Size = %d\n" );
...
//					     ptr_frame_info=get_frame_info(e->env_page_directory, i, &ptr_table);
